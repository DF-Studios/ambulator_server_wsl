...
if [ -z "$(sudo docker ps -a -q -f name=tabletop_server)" ]; then
    ...
    sudo docker run --name tabletop_server -e POSTGRES_USER=tabletop -e POSTGRES_PASSWORD=$POSTGRES_PASSWORD -e POSTGRES_DB=tabletop_server -d postgres
else
...
if [ -z "$DATABASE_URL" ]; then
    export DATABASE_URL="postgres://tabletop:$POSTGRES_PASSWORD@localhost:5432/tabletop_server"
